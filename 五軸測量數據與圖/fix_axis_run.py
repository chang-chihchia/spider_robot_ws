...
def main(args=None):
    ...
    # 初始化 CSV
    csv_file = open('/home/chia/spider_ws/experiment_data.csv', 'w', newline='')
    writer = csv.writer(csv_file)
    writer.writerow(['time', 'actual_x', 'actual_y', 'actual_z', 'ref_x', 'ref_y', 'ref_z', 'pitch', 'roll'])
            ...
            for i in range(1, 7):
                ...
                if i == 1:
                    if elapsed > 2.0:
                        ...
                        # 取得當前高度
                        heights = [a.joint_cur_pos.get(leg*3+3, {}).get('pos', 0.0) for leg in range(6)]
                        
                        # 計算原始偏差
                        raw_pitch = (sum(heights[0:3]) - sum(heights[3:6])) * 0.05
                        raw_roll = (heights[0] + heights[5] - heights[2] - heights[3]) * 0.05
                        
                        # 【關鍵修正】：減去一個初始參考值或是當前的平均水平，消除累積漂移
                        # 這裡我們假設起始的 2 秒是穩定的校準階段
                        # 如果你有更精確的「靜止零點」，請直接替換為那個數值
                        stable_pitch_offset = 0.0 # 若機器人有持續傾斜，可在此手動調整補償
                        stable_roll_offset = 0.0
                        
                        pitch = raw_pitch - stable_pitch_offset
                        roll = raw_roll - stable_roll_offset
                        
                        # 寫入檔案
                        writer.writerow([elapsed, curr_pos[0]*1000, curr_pos[1]*1000, curr_pos[2]*1000, 
                                         target_xyz[0]*1000, target_xyz[1]*1000, target_xyz[2]*1000, 
                                         pitch, roll])
